...
sed 's#^exec python3 /app/server.py$#echo BOOT_COMPLETE#' entrypoint.sh > "$STUB"
chmod +x "$STUB"
...
check "says it left it untouched" "$(echo "$OUT" | grep -q 'leaving it untouched' && echo 1)"
...
check "reports the snapshot restore" "$(echo "$OUT" | grep -q 'from credguard snapshot' && echo 1)"
...
check "clearing is logged" "$(echo "$OUT" | grep -q 'clearing orphaned lock' && echo 1)"
...
check "warns that auth is absent" "$(echo "$OUT" | grep -q 'no usable credentials.json' && echo 1)"
...
check "warns the seed is stale" "$(echo "$OUT" | grep -q 'probably already superseded' && echo 1)"
